...
abstract contract CreateXScript is Script {
    ...
    function setUpCreateXFactory() internal {
        if (!isCreateXDeployed()) {
            ...
            if (!isCreateXDeployed()) {
                revert(
                    "\u001b[91m Could not deploy CreateX! \u001b[0m"
                );
            }
        } else {
            console2.log(
                "\u001b[92m\u2714 CreateX already deployed on chain:",
                block.chainid,
                "\u001b[0m"
            );
        }
    ...
    function deployCreateX() internal {
        ...
        console2.log(
            "\u2692 Etching missing CreateX on chain:", block.chainid
        );
    ...
    function computeCreate3Address(
        ...
        bytes32 guardedSalt = keccak256(
            abi.encodePacked(uint256(uint160(deployer)), salt)
        );

        return CreateX.computeCreate3Address(
            guardedSalt, CREATEX_ADDRESS
        );
    }
