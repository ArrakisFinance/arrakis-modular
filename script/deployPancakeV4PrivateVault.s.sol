...
import {CurrencyLibrary} from
    "@pancakeswap/v4-core/src/types/Currency.sol";
import {
    PoolId,
    PoolIdLibrary
...
import {PancakeSwapV4Oracle} from
    "../src/oracles/PancakeSwapV4Oracle.sol";
...
address constant token1 = 0x55d398326f99059fF775485246999027B3197955;
uint24 constant fee = 500;
int24 constant tickSpacing = 10;
address constant hooks = address(0);
uint160 constant sqrtPrice = 4_071_912_024_527_618_728_846_590_638_494;

bool constant isInversed = false;

bytes32 constant salt = keccak256(
    abi.encode("BSC ETH/USDC Pancake swap V4 private vault beta 3")
);
...
OracleDeployment constant oracleDeployment =
    OracleDeployment.PancakeSwapV4Oracle;
...
uint8 constant token1Decimals = 6;
...
contract DeployPancakeV4PrivateVault is CreateXScript {
    ...
    function run() public {
        ...
        address panV4PrivateUpgradeableBeacon = getUpgradeableBeacon();
        ...
        if (createPool) {
            ICLPoolManager(poolManager).initialize(poolKey, sqrtPrice);
        }
        ...
        if (oracleDeployment == OracleDeployment.PancakeSwapV4Oracle)
        {
            oracle = address(
                new PancakeSwapV4Oracle(poolManager, isInversed)
            );
        ...
        address vault = IArrakisMetaVaultFactory(factory)
            ...
            token1,
            vaultOwner,
            panV4PrivateUpgradeableBeacon,
            moduleCreationPayload,
            initManagementPayload
        );

        console.logString("Pancake Private Vault Address : ");
    ...
    function getUpgradeableBeacon() internal view returns (address) {
        ...
        // mainnet
        if (chainId == 56) {
            return 0xC164893891d312876C8B0A59811DB096f8a740Cc;
        } else {
