...
contract VaultCreatorActor {
    ...
    function createPrivateVault(
        ...
        bytes32 salt = keccak256(abi.encodePacked(vaultCounter++, seedValue, "private"));
        
        // Select random tokens (ensure token0 < token1)
        uint256 token0Index = seedValue % availableTokens.length;
        uint256 token1Index = (seedValue + 1) % availableTokens.length;
        if (token0Index == token1Index) {
            token1Index = (token1Index + 1) % availableTokens.length;
        }
        
        address token0 = availableTokens[token0Index];
        address token1 = availableTokens[token1Index];
        ...
        address beacon = availableBeacons[seedValue % availableBeacons.length];
    ...
    function createPublicVault(
        ...
        // Generate deterministic but varied configurations
        bytes32 salt = keccak256(abi.encodePacked(vaultCounter++, seedValue, "public"));
    ...
    function _generateRandomSetupParams(
        ...
        uint256 seed1 = uint256(keccak256(abi.encodePacked(seedValue, block.timestamp)));
        uint256 seed2 = uint256(keccak256(abi.encodePacked(seed1, block.number)));
        uint256 seed3 = uint256(keccak256(abi.encodePacked(seed2, msg.sender)));
        
        return SetupParams({
            vault: vaultAddress,
            oracle: IOracleWrapper(availableOracles[seed1 % availableOracles.length]),
    ...
    function getCreatedVaultsCount() external view returns (uint256) {
        return createdVaults.length;
    }
    
    /// @notice Returns a specific created vault by index
    function getCreatedVault(uint256 index) external view returns (address) {
    ...
    function updateAvailableTokens(address[] calldata newTokens) external {
    ...
    function updateAvailableBeacons(address[] calldata newBeacons) external {
    ...
    function updateAvailableOracles(address[] calldata newOracles) external {
