...
contract ArrakisPublicVaultMock is ERC20{
    ...
    function setAmountToTake(uint256 amount0_, uint256 amount1_) external {
        amount0ToTake = amount0_;
        amount1ToTake = amount1_;
    }

    function setAmountToGive(uint256 amount0_, uint256 amount1_) external {
    ...
    function mintLPToken(address receiver_, uint256 amount_) external {
    ...
    function mint(uint256, address) external payable returns(uint256 amount0, uint256 amount1) {
        amount0 = amount0ToTake;
        amount1 = amount1ToTake;
        if (address(token0) == NATIVE_COIN) {
            require(amount0ToTake == msg.value);
        } else
            token0.transferFrom(msg.sender, address(this), amount0ToTake);
        if (address(token1) == NATIVE_COIN) {
            require(amount1ToTake == msg.value);
        } else
            token1.transferFrom(msg.sender, address(this), amount1ToTake);
    }
