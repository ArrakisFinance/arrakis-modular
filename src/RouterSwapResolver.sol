...
contract RouterSwapResolver is IRouterSwapResolver {
    ...
    function calculateSwapAmount(
        ...
        (uint256 gross0, uint256 gross1) = _getUnderlyingOrLiquidity(vault_);
        if (gross1 == 0) {
        ...
        if (amount0In_ > 0 && amount1In_ > 0) {
            (uint256 amount0, uint256 amount1, ) = router.getMintAmounts(
                address(vault_),
                amount0In_,
                amount1In_
            );
        ...
        uint256 factor0 = 10 **
            (18 - IERC20Metadata(vault_.token0()).decimals());
        uint256 factor1 = 10 **
            (18 - IERC20Metadata(vault_.token1()).decimals());
        ...
        uint256 value0To1Left = (amount0Left * factor0 * price18Decimals_) /
            1 ether;
