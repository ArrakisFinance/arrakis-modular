...
import {IVelodromeStandardModulePrivate} from
    "../interfaces/IVelodromeStandardModulePrivate.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    bytes32 public constant id =
        0xc9dc553fecad110de03a502cd9ae07aa1331ce7887ef847b047076558c6d73ee;

    /// @inheritdoc IVelodromeStandardModulePrivate
    ...
    uint256 internal _veloManagerBalance;
    ...
    function initialize(
        ...
        if (_token0 == VELO || _token1 == VELO) {
            revert VELOTokenNotSupported();
        }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        for (uint256 i; i < tokenIds.length;) {
            ...
            (uint256 amt0, uint256 amt1, uint256 veloCo) =
                _decreaseLiquidity(modifyPosition, sqrtPriceX96);

            amount0 += amt0;
            amount1 += amt1;
            veloAmountCollected += veloCo;
        ...
        _veloManagerBalance += FullMath.mulDiv(
            veloAmountCollected, _managerFeePIPS, PIPS
        );
    ...
    function claimRewards(
        ...
        veloBalance +=
            IERC20Metadata(VELO).balanceOf(address(this)) - balance;

        // #region take the manager share.

        _veloManagerBalance +=
            FullMath.mulDiv(veloBalance, managerFeePIPS, PIPS);

        // #endregion take the manager share.

        uint256 veloToClaim = IERC20Metadata(VELO).balanceOf(
            address(this)
        ) - _veloManagerBalance;

        IERC20Metadata(VELO).safeTransfer(receiver_, veloToClaim);

        emit LogClaim(receiver_, veloToClaim);
    }
    ...
    function setReceiver(
        ...
        address oldReceiver = veloReceiver;
    ...
    function claimManager() public nonReentrant whenNotPaused {
        ...
        uint256 amountToSend = _veloManagerBalance
            + FullMath.mulDiv(veloBalance, managerFeePIPS, PIPS);
        _veloManagerBalance = 0;

        // #endregion take the manager share.

        address _veloReceiver = veloReceiver;

        IERC20Metadata(VELO).safeTransfer(_veloReceiver, amountToSend);

        emit LogManagerClaim(_veloReceiver, amountToSend);
    }
        ...
        if (params_.swapPayload.amountIn > 0) {
            ...
            if (
                params_.swapPayload.router == address(metaVault)
                    ...
                    || params_.swapPayload.router == VELO
            ) {
                revert WrongRouter();
            }
    ...
    function veloManagerBalance() external view returns (uint256) {
        ...
        for (uint256 i; i < length;) {
            ...
            veloBalance += ICLGauge(_gauge).rewards(tokenId);
            veloBalance +=
