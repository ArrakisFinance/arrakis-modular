...
contract ArrakisStandardManager is IArrakisStandardManager, Ownable {
    ...
    uint24 public immutable defaultFeePIPS;
    ...
    function decreaseManagerFeePIPS(
        ...
        if (!_vaults.contains(vault_)) revert NotWhitelistedVault(vault_);
        uint24 oldFeePIPS = vaultInfo[vault_].managerFeePIPS;
    ...
    function submitIncreaseManagerFeePIPS(
        ...
        if (!_vaults.contains(vault_)) revert NotWhitelistedVault(vault_);
        if (pendingFeeIncrease[vault_].submitTimestamp != 0) revert AlreadyPendingIncrease();
        if (vaultInfo[vault_].managerFeePIPS >= newFeePIPS_) revert NotFeeIncrease();
    ...
    function finalizeIncreaseManagerFeePIPS(
        address vault_
    ) external onlyOwner {
        FeeIncrease memory pending = pendingFeeIncrease[vault_];

        if (pending.submitTimestamp == 0) revert NoPendingIncrease();
        if (block.timestamp <= pending.submitTimestamp + WEEK) revert TimeNotPassed();
    ...
    function rebalance(address vault_, bytes[] calldata payloads_) external {
        ...
        if (info.cooldownPeriod + info.lastRebalance >= block.timestamp) revert TimeNotPassed();

        // #region get current value of the vault.

        (uint256 amount0, uint256 amount1) = IArrakisMetaVault(vault_)
            .totalUnderlying();
        /// NOTE what about native token case ?
        ...
        if (currentSlippage > info.maxSlippagePIPS) revert OverMaxSlippage();
    ...
    function announceStrategyData(address vault_, bytes calldata data_) external {
        if (!_vaults.contains(vault_)) revert NotWhitelistedVault(vault_);
        VaultInfo memory info = vaultInfo[vault_];
        if (info.stratAnnouncer != msg.sender) revert NotStratAnnouncer();

        emit LogStrategyData(vault_, data_);
    }

    function updateVaultInfo(SetupParams calldata params_) external {
        if (!_vaults.contains(params_.vault)) revert NotWhitelistedVault(params_.vault);
        address o = IOwnable(params_.vault).owner();
        if (msg.sender != o) revert OnlyVaultOwner(msg.sender, o);
